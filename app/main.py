...
async def get_candles(pair):
    if pair not in candles_streams:
        candles_streams[pair] = await instance.subscribe_candles(pair, 1)
    async for candle in candles_streams[pair]:
        print(f'par: {pair} candle: {candle["open"]}')
        for trade_info_id in trade_info_ids:
            ...
            if candle['open'] == price:
                if trade_status == 0:
                    ...
                    await instance.buy_digital_spot(active=pair, amount=amount, action=action,
                                                  duration=time_frame)
...
async def monitor_pairs():
    while status_bot == 1:
        ...
        for pair in monitored_pairs:
            ...
            asyncio.create_task(get_candles(pair))
...
async def main():
    ...
    while True:
        await asyncio.gather(monitor_pairs(), asyncio.sleep(5))
