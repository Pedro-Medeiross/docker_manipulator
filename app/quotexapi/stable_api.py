...
class Quotex(object):
    ...
    def connect(self):
        if global_value.check_websocket_if_connect:
            self.close()
        self.api = QuotexAPI("qxbroker.com",self.email,self.password,)
    ...
    def get_historico_close(self):
        return self.api.get_historico_close
    ...
    def get_balance(self, tipo_conta = "PRACTICE"):
        ...
        return float(f"{truncate(balance, 2):.2f}")
    ...
    def buy(self, price, asset, direction, duration):
        ...
        while not self.api.buy_id:
            #time.sleep(5)
            #print('stable_api.py : ', self.api.buy_id)
            status_buy = True
            break
        else:
            status_buy = True
